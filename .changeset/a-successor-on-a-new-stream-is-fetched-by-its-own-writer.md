...
- **A promotion onto a generation on another stream stops folding the incumbent**, however it arrived, so the old stream's fetcher stops once nothing here reads it (no more chain calls for it). A replaced new-stream successor stops being fetched the same way. The incumbent is retained as `predecessor`; a revert onto it is the freeze a revert across a filter change always was.
