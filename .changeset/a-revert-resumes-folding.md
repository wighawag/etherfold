...
- `GenerationInstantiationError`: an instantiation that fails (no stored bundle, the host refused the bytes, they hash to another identity, or the fold is on another stream) REFUSES the move. The pointer stays where it was and the generation that answered reads still folds.
