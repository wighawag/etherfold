...
`stream.alwaysFetchTimestamps` STAYS, because two cases survive any version bump and neither improves with time: a node being FORKED that predates the spec change keeps the field absent rather than defaulting it (EDR's `Option<u64>` is deliberate, so a missing timestamp stays distinguishable from a real one), and EDR's on-disk RPC response cache is version-segmented, so entries written before the change keep answering without it until `rpc_cache` is dropped.
