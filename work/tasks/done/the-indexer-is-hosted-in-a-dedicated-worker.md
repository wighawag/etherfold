...
**The envelope's shape.** One `{protocol, kind, id, case, payload}` request and one `{protocol, kind, id, case, ok, value|error}` response, with the surfaces declared in a `PortCases` map (case name → `{request, response}`) that a later task adds a KEY to. Alternatives considered: a channel per surface (rejected by ADR-0082), and Comlink-style proxying (the ADR allows it; rejected because the contract is then implicit in a proxy's behaviour instead of in named message shapes a test can assert). The `case` is carried on the response too, so a refusal names the surface rather than an opaque id. Touches every later task on this port.

**No protocol VERSION, only a namespace.** `protocol: 'etherfold/indexer-port'` exists so a host ignores messages another library posted to a shared scope. A version number was considered and rejected: both ends come out of one build (the app's bundler compiles the tab and the worker entry from the same tree), so a version would be a compatibility promise nothing keeps. Touches the SharedWorker task, where one host serves several tabs of the same origin — still one build.

**Where the host/container seam falls.** The host is a NEW function (`serveIndexerHost`) that opens the container directly through `openIndexer`, not a wrapper around `createIndexerState`. It takes `BrowserGenerationSpec` — the exact shape `createIndexerState` already takes — plus what that function takes at `init`, so the last task can fold the two together rather than reconcile two vocabularies. What it deliberately does NOT carry yet: the scheduled prune, the stream-seed install, generation-progress reporting, demotion publication, the reactive triple. Alternative considered: moving `createIndexerState` wholesale now (rejected — the prompt fences it, and it would decide six later tasks by accident). Touches all six later tasks, especially `createindexerstate-becomes-the-main-thread-host`.

**The host auto-starts its driver, and stops on a failure waiting cannot fix.** There is no control surface yet, so a host that needed to be started could not be. It retries `isRetryable` failures on a one-second timer and stops otherwise, reporting `failure` through `progress` rather than throwing (an exception in a worker entry reaches nobody). This makes `start` in `a-tab-controls-the-indexer-across-the-port` an idempotent no-op on a fresh host unless that task changes the default. Alternative: start lazily on the first port call (rejected — it would make "how far have you got" a mutating verb).

**A new type name, `HostAccess`.** The glossary has HOST, CONTAINER, DRIVER, PORT and HOSTING SHAPE but no word for the thing a shape produces. `HostAccess` is `{host, endpoint, close?}` and is documented as exactly `CONTEXT.md`'s phrase "how a port is obtained". It does not re-mean PORT: the port is the typed boundary built OVER it. Alternative considered: overloading `connectToIndexerHost({worker})` with a discriminated options object (rejected — it would put the word `Worker` inside the shape-agnostic module and make the seam a branch rather than a file).

**The clone guard refuses class instances, which structured clone accepts.** Clone copies own properties and drops the prototype, so a store/processor/provider "arrives" and fails on its first call one layer from anything explaining it. Refusing is stricter than the platform and is the point. Risk: a later surface wanting to carry a class-shaped value must send its data instead. Touches the store-proxy task, whose rows are plain objects.

**`HostProgress` carries no derived figure, and reports the container's `0 of 0` honestly.** `blocksBehind` already means "behind the CANONICAL generation" (`GenerationProgress`), so reusing it for "behind the tip" would give one word two meanings; the two raw numbers cross and the derivations are the push task's to place. Consequence documented in the JSDoc and found the hard way: before the first fetch a cursor is `0 of 0`, so equality alone is not "caught up" — it made the Chromium run race the fold until the tests named the fixture's tip.

**`scope` is reported alongside `host`.** `host` is a label the entry passes; `scope` is `globalThis.constructor.name` measured where the answer was computed. It exists so "the UI thread is not doing the fold" is assertable without a timing threshold. Nothing branches on it.

**Two new refusals.** `hostIndexerInThisWorker` throws if called in a document (a stray import would start a second container on the UI thread folding into the same store), and again if the context has no worker messaging. `UnclonableValueError` is a new exported error type (a `TypeError`: the same value is refused identically on every attempt).
