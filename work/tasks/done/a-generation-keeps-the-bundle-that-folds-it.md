...
- **Port shape: bytes go on the write, reads are a separate call.** The bytes travel as `GenerationRegistryWrite.bundle`, alongside `put` rather than inside the record, plus `readBundle(id)` on the port and `bundleOf(id)` on the registry. They stay off the record because the IndexedDB port stores `write.put` exactly as given, so bytes on the record would be stored by the one runtime that must never keep code. They stay out of `read()` because every commit decision reads that, and none needs a whole processor's bytes. I considered an optional capability on the port instead of a required method; a required `readBundle` that the browser answers with `undefined` is more uniform. This touches every `GenerationRegistryPort` implementation, including the test ports.
- **Deletion is one path because the bytes live on the registry row.** Removing the row is removing the bytes, in the same SQL statement and the same memory map entry. So a reclaim, a replaced successor and a drop on promotion all delete the bytes through `deleteGeneration`, and none of them mentions bytes. The alternative was a table inside the generation's state namespace, deleted by the host's `dropState`. I rejected it for three reasons:
  - ADR-0053 keeps registry rows out of that namespace.
  - `dropState` runs after the registry commit, so it is a second step that can fail on its own.
  - Registration and bytes would stop being atomic.

  Because ADR-0092's "One namespace then holds everything" read as the state namespace, I corrected that paragraph in place and said so in the text, following the ADR-FORMAT rule for text that never described code. Where the column lives is on-disk format, which costs little to change while nothing is published.
- **A registration that resolves an existing generation writes no bytes.** The identity is the hash of the bytes, so any valid resend is identical, and re-sending a processor's worth of bytes on every restart buys nothing. The cost is that a row registered earlier without bytes is never filled in later; no such rows exist, because nothing is published.
- **The SQL column is nullable.** A NULL is the natural way to express "this generation's code is gone" (the spec's story 8, owned by `a-generation-says-whether-it-can-run-here`), and bare registry tests still register without bytes. Bytes are enforced where Node actually registers: the receiving container's `add`. I considered NOT NULL enforced by the substrate; it would have been stricter but would have forced bytes into about 40 registry-bookkeeping tests.
- **`resolveGeneration` no longer registers anything.** It was a second registration route on the Node container that carried no bytes. It now resolves generations already registered and throws `UnknownGenerationError` otherwise; I updated the `GenerationContainer` docs to match. Two core tests used it to create a generation in no slot. They now call `registry.create(id, {bundle})` directly, with bytes.
- **The test seam `processorIdentity?: string` became `processorBundle?: Uint8Array`.** This covers `IndexingDependencies`, `IndexDependencies` and the reconfigure context. A substituted arrival now states the bytes it stands for; its identity is derived with `processorArtifactIdentity` and the bytes are stored like a real bundle's. Bytes read from disk still win. I considered keeping the name and adding bytes beside it, but those are two values that can disagree. This changes a published (test-only) type in `etherfold`, and the changeset says so.
- **An empty `Uint8Array` is refused as a bundle,** both by the registry and by the receiving container, because it is no processor at all.
