...
- **Deleted the root `TODO.md` file outright, not just its line.** It held exactly one item, the one this task closes by deciding not to build it, so removing the line would have left a zero-byte file in the repo root. Alternative considered: keep an empty `TODO.md`, or keep it with a "nothing outstanding" placeholder; both preserve a file whose whole content is gone, and this repo tracks work in `work/`, not in a root TODO. Touches: two docs that referenced "the root `TODO.md`" (see next two entries). Reversible in one commit if a human wants the file back.
- **Annotated `docs/reviews/todo-triage.md` as closed-by-deletion rather than leaving it silent** (the task offered either). Both of its `logValues` entries (the root-TODO item and the "type `logValues`" standalone cleanup) now carry a note pointing at `work/specs/tasked/the-stream-stores-only-what-the-node-said.md`, and the dated triage text itself is left verbatim underneath, because it is a record of what was true when it was written. It is also where the "flag STUB" evidence lives, so a silent leave would have had it still recommending a feature this task decided against.
- **Touched `docs/design/historical-state-database.md` (one line), which is outside the task's enumerated sweep.** That line said raw `data`/`topics` retention "is mutually exclusive with the root `TODO.md` idea of trimming log fields via stream config" — a live design doc pointing at a file I was deleting, so it would have dangled. Rewritten to say the conflict is settled by the deletion. Alternative considered: leave it and accept the dangling reference; rejected for exactly the reason this task exists (a live doc explaining today's design by something that no longer exists).
- **The type test's refusal literal spells out EVERY raw-log flag, rather than a partial `{topics: true}`.** `LogValuesFlags` was a mapped type with all-required properties, so a partial literal was already a type error today: the `@ts-expect-error` would have been satisfied before and after the deletion and proved nothing. The full literal was VALID before (hence RED, verified) and fails only on the unknown property after. Touches nothing else; noted because the obvious shorter form is a silently useless assertion.
- **The guard's load-path test drives the REAL `reparse`.** The shared stream-cache world (`test/utils/streamCacheWorld.ts`) replaces the log fetcher wholesale and its `reparse` is a pass-through, so the clear branch is unreachable through it as-is. Rather than change that shared fake (it is used by five other suites, whose assertions are about cache arithmetic), the new test overlays a real `LogEventFetcher.reparse` onto the fake fetcher for its two cases. Alternative considered: give the shared world a real `reparse`; rejected as a wider blast radius than this task needs.
- **No ADR written.** The durable "why" for deleting rather than relocating the knob already lives in the source spec and is now restated in the changeset; nothing here is hard to reverse or surprising without that context, so it does not meet the ADR gate.
