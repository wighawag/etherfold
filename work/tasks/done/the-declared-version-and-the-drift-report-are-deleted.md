...
**An unnameable MODULE arrival is REFUSED, in `@etherfold/browser`.** `moduleProcessorIdentity` now returns `string` and throws when `getCodeFingerprint()` declines (all handlers `bind`-ed or proxied). *Why:* the other three options are all lies — hashing `[native code]` into a constant no edit can move, taking a name from the application (the author-declared identity through the one door ADR-0086 leaves open), or letting `undefined` reach `GenerationId.processor`. The refusal is loud, lands before anything is registered or any state claimed, and names the two ways out. *Alternatives:* leave it `undefined` and let core's structural guard fire (rejected: core cannot say WHY, and the message is what an app author needs); accept `processorIdentity` on `updateProcessor` (rejected explicitly by ADR-0086 and by the task that built the arrival). *Touches:* `@etherfold/browser`'s `updateProcessor` and `createIndexerState`/`serveIndexerHost`; the derivation is computed before the auto-index loop is paused, so a tab meeting the refusal is left exactly as it was. Pinned by `aModuleIsIdentifiedByItsHandlerSources.test.ts`.

**A `--processor` path naming an UNBUNDLED entry point is REFUSED, minimally, at arrival resolution (`requireArrivalIdentity`, `etherfold`).** *Why:* deleting the declared identity leaves such a deployment with no name for its fold, and this task may not leave one running unnameable. *Touches `a-path-naming-an-unbundled-entry-point-is-refused` directly:* that task is still correct and still needed — it moves this refusal to CONFIGURATION RESOLUTION in ADR-0048's shape and gives it the build command an author needs. I deliberately did NOT name a build command (the wording is `the-build-command-and-its-pinning-rule-are-documented`'s), and I did not put the refusal in `openProcessorArrival` (`@etherfold/utils`), because that would also refuse the injected-arrival test seam and would put a configuration decision inside a loader. *Alternative rejected:* deriving a CLI identity from handler sources — that is the production identity `fingerprint.ts` argues against, and it would give the same code two names depending on how it was loaded.

**`GenerationSpec.processorIdentity` stays typed optional but is REFUSED when absent.** *Why:* `add` reads state → processor → THEN identity, which is exactly what makes a module arrival expressible (the browser fills the field in from inside `createProcessor`). A required field would be unstatable there. The refusal (`requireProcessorIdentity`, `@etherfold/core`) is the structural backstop under both arrival-level refusals above.

**The `identity` option on `EntityEventProcessor` / `VersionedStateEventProcessor` is DELETED, not kept.** It existed solely to answer `getVersionHash()`. A fold that computes no identity of its own has nothing for a caller to override, so the "two live answers" hazard it closed no longer exists. *Touches:* every host now passes ONE value to the table namespace (ADR-0053), the registry and the engine, and none to the processor.

**DEVIATION FROM ADR-0086's consequence list, recorded as the task asked.** The ADR says `getCodeFingerprint()` and `utils/fingerprint.ts` go. They do not: the browser's module arrival names its fold by the SEAM method (not the standalone function, because the seam delegates correctly through a wrapper), so deleting it would leave the one arrival with no bytes with nothing to be. `fingerprint.ts` now records this the way it already recorded its deviation from ADR-0008, and the changeset states it.

**The pending changeset `a-reload-that-changed-nothing-reports-processor-drift.md` is DELETED, not amended.** *Why:* nothing has been released, so it would otherwise ship a release note announcing a report that never existed outside this repository. A changeset is a release note, not a history log; the history lives in the work record and in ADR-0086. My changeset states the deletion and the reasoning, so the retirement is explained in the notes that DO ship. *Alternative:* amend it to "shipped and retired" — rejected as noise about a release nobody saw.

**ADR-0086's `status:` front matter is left at "accepted, not yet implemented"**, matching the repo's practice (ADR-0084 is implemented and still says the same); flipping it is not this task's convention to invent.
