...
**Where the runtime's answer to "does this registry assign a predecessor" lives: an OPTION ON THE MOVE, `moveCanonicalTo(id, {assignPredecessor?: boolean})`, defaulting to `true`, passed by `Indexer.movePointerTo` and by nothing else.** Why: the distinction is about the CONTAINER doing the moving, not about the substrate holding the rows. The same chain-facing `Indexer` runs over a memory registry in core's tests and a durable IndexedDB one in a tab, and the receiving container runs over SQL; keying the answer to the registry would make the two independently settable and a mismatch expressible (a registry that assigns handed to a container that must not). Keying it to the call makes the container the sole authority and keeps the default on the safe side, so any caller that says nothing keeps its revert window. Alternatives considered: (a) a parameter on `openGenerationRegistry(port, caps, ...)`, which the task named as the place the runtime difference is supplied today, rejected for the mismatch above and because it puts a per-move fact on an object-lifetime seam; (b) a field in `GenerationCaps`, ruled out by the task and by that type's own docs (a COUNT, never a policy); (c) a `dropOnPromotion` default change, rejected by the ADR itself (different concept, and a second act after the move). What it touches: `GenerationRegistry.moveCanonicalTo`'s public signature (hence the `minor` changeset on `@etherfold/core`), and nothing else — `ReceivingIndexer.movePointer`, the admin pointer route, `SLOT_NAMES`, `displacedBySuccessor` and the caps are all unchanged.

**The chain-facing drop-log message stops claiming a revert target.** `dropReplaced`'s log said a replaced successor was safe to drop because it was "neither the canonical generation nor what `predecessor` holds". On this runtime the second clause can never be the reason, so it is removed rather than left to read as though a revert target existed here. The shared safety rule (`displacedBySuccessor`, which still refuses to touch a generation ANY other slot names) is unchanged; this is wording at one runtime's call site. Touches: the chain-facing container's log text only.

**The in-session "way back" assertions were kept but re-justified rather than deleted.** In the core and browser suites, moving the pointer back to the superseded generation still works while a session holds its fold — but it now works because the generation survives as the stream's FETCHER, not because a slot protected it. I kept the assertions and named the real reason in the comments, so the next reader does not read them as evidence of a revert window this runtime no longer has.
