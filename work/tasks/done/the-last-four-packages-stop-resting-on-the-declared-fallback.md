...
**Added a NEW labelled witness in `packages/browser` rather than leaving `liveReload.test.ts` as one.** `liveReload`'s four failures came from a fake whose `getCodeFingerprint()` returned `undefined`, so `moduleProcessorIdentity` declined and the core fell back. `test/utils/fakeGeneration.ts` called that omission deliberate — but its own words were "Until [`a-module-handed-to-a-tab-is-identified-by-its-handler-sources`] lands, those cases keep the author-declared fallback", and that task HAS landed, so the deferral had expired and the cases are remainders (their subjects are reconfigure serialization and `{force}` passthrough, not identity). Migrating them, however, would have deleted the only live exercise of a **production** dependency: `src/moduleIdentity.ts` documents that `undefined` is a real answer and that the caller then falls back, calling itself "the last place that fallback is reachable in this package, and one `the-declared-version-and-the-drift-report-are-deleted` has to answer for". So I migrated the four and wrote one focused case that pins that branch (an author object with every handler `bind`-ed, so `processorCodeFingerprint` sees only `[native code]`). **Alternatives:** (a) leave all four as witnesses — rejected, four subject-mismatched cases give the contract task a muddle instead of a decision, and the stale doc would have stayed stale; (b) migrate and add nothing — rejected, the contract task would then delete a fallback production still takes with no test going red, which is precisely the failure this family keeps repeating. **What it touches:** `the-declared-version-and-the-drift-report-are-deleted` inherits one explicit decision ("what is an unnameable module called once there is no declared hash?") instead of a silent branch. **Coherence:** no new concept; it reuses `moduleProcessorIdentity` and the existing witness vocabulary.

**`liveReload.test.ts`'s fake now names itself by a marker-derived `getCodeFingerprint()` instead of by a moving `getVersionHash()` string.** That IS the module arrival's derivation (`moduleProcessorIdentity` is `getCodeFingerprint()` and nothing else), so the fake got more realistic rather than less, and the swap semantics are identical: same marker = same fold = declined, different marker = swapped. Two consequences are visible in the file: the recording harness's `updateProcessor` wrapper now **forwards its options** (it dropped them, so the identity the hook derived never reached the core — a genuine fidelity bug the probe exposed, and the reason one case failed even after `init` was fixed), and the `#5` assertion is now `toEqual({force: true, processorIdentity: makeProcessor().getCodeFingerprint()})`. **Alternative:** supply `processorIdentity` through the spec — rejected, `IndexerState.updateProcessor` deliberately accepts no identity from an app, so it would have fixed `init` and not the swap. **What it touches:** nothing outside the suite; no production code changed.

**The two `lifecycle.test.ts > the identity this fold answers with` cases are witnesses, not remainders.** Their subject is the arrival's identity, but each proves it by contrast with a fold built with no identity (`expect(handed…).not.toBe(declared.getVersionHash())`, and `declared.configure(…)` MOVES the declared hash). Migrating would mean deleting the contrast, i.e. deleting coverage. Per the prompt's tie-break I labelled rather than migrated. **What it touches:** the contract task deletes the `declared` half of each case and keeps the arrival assertion above it, which stands alone.

**`@etherfold/conformance-workload-stratagems` is deliberately absent from the changeset** even though it was edited: it is `private: true` and `.changeset/config.json` sets `privatePackages: false`, so naming it is the exact mixed-changeset refusal `scripts/check-changesets.mjs` exists to catch. Its share of the change is described in the changeset body instead.

**No `docs/spikes/` artifact.** The probe is a scratch `dist/` edit the task explicitly says not to commit, and its recipe and numbers are already recorded in the two observation notes and in the changeset, which is where the contract task will look.
