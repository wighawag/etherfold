...
- **The stripped-cursor type lives INTERNAL and unexported, not in `types.ts`.** `storedLastSyncOf` needs a return type whose window says STORED, so I minted `StoredLastSync` / `StoredEventBlock` in `packages/core/src/internal/stream/strip.ts` rather than beside `StoredLogEvent` in the public types. Alternative considered: exporting it from `@etherfold/core` now. I rejected that because the *next* task (`the-stream-seam-takes-only-the-stored-event`) explicitly owns the choice between "leave the seam's window typed decoded" and "give the seam a stored-window variant", and publishing the name here would pre-empt it and grow the public API for a shape nothing outside core can use yet. Touches: that task, which may promote or delete these two types. They carry no ABI type parameter deliberately — the ABI is what the decoded half was made under.
- **The strip module is a new file under `internal/stream/`, not a function in `internal/engine/utils.ts` and not in `src/stream/`.** `src/stream/*` is entirely re-exported from the package index, so putting it there would have published it; `internal/engine/utils.ts` is the fold's grab bag and this is a storage-boundary rule. Touches: only the import in `indexer.ts`.
- **`context` is carried over by reference into the stripped cursor.** Nothing is stripped from it, so copying would buy an allocation and no guarantee, and handing the whole `lastSync` over already shared it. Alternative considered: a defensive clone, rejected as scope creep with no failure it prevents (the load path's `context.source` mutation predates and is unaffected by this change).
- **The strip runs AFTER `streamRemainderOf`, not before.** So the retractions that helper synthesises from `streamWrittenNotProcessed` are stripped too, and its `sameEvent` comparison keeps reading the decoded in-memory list. Order is not observable today (the comparison reads raw fields only), so it is pinned by a test rather than by a comment.
