...
- **How the fetchers stay in step with the slots.** The container decides which streams are fetched and the CLI only mirrors it. `fetchedStreams()` lists every stream read by a held fold of a registered generation. Before every cycle, `StreamFetchers.reconcile()` starts a fetcher for each new stream on that list and stops the fetcher for any stream that left it. The folds follow the slots: a successor is held from the moment it registers, a promotion across streams stops folding the incumbent, and a replaced successor is dropped. This touches `liveIngestions`, which now uses the same logic. The alternative was for the CLI to work out fetch streams from the slots itself, which would be a second answer that could drift from the container's.
- **The one cycle runs every fetcher in turn, oldest first.** It reports whichever result most needs action (fatal, then retry, then still catching up, then contended, then caught up, then idle). It waits the shortest delay any fetcher's own backoff asks for. With one stream it behaves exactly like a single fetcher. This is why `runFetcherLoop` now takes a `CycleRunner`. The alternative was one concurrent loop per stream, which would run the rebuild, prune and promotion step concurrently. For `build`, this ordering means it stops at the tip only once every stream it fetches has reached it.
- **If the container ever reports no streams at all while fetchers are running, the fetchers are kept.** The next push is then refused as `NoLiveReceiverError` before any chain call, as before. The alternative was to stop them all, which would leave nothing to report the problem and would break `build`'s exit on that refusal.
- **A promotion across streams stops folding the incumbent however it arrived.** Before, only a fold built from stored bytes was stopped. Same-stream promotions keep their old behaviour, because there it costs no extra fetch. The alternative was to always stop the superseded fold, which would change many same-stream revert paths. One consequence: the split `index` receiver also stops folding a cross-stream incumbent at promotion, so its old stream's writer stops accepting pushes.
- **A pointer move onto a generation whose stream nothing here fetches freezes, whatever source the instantiation names.** Startup does not apply this rule, so the canonical generation and a pending successor always fold. This keeps "a revert across a filter change freezes" true, including a revert onto a configured source's stream after the fetch has moved off it.
- **A node whose source came from its processor module builds a stored generation over its bundle's own contracts** (`sourceCarriedByBundle`, passed from `run`/`build` only). Without it, restarting the node that took the upload, with the processor it was started with, would freeze the new canonical generation, and a successor still catching up would never be fetched. A source the operator configured still overrides, and `index` is unchanged. This is recorded in ADR-0093's amendment.
- **One gap is left as is.** Restarting a configured node with its original `--processor` after a cross-stream promotion holds that processor's fold as the predecessor (existing behaviour, since `add` holds whatever arrives), so the old stream keeps being fetched while that process runs. That is consistent with the rule "fetched while a fold here reads it". Changing what a configured arrival does when it names the predecessor is the ADR-0093 amendment's territory, not this task's.
- **`fetchedSource` follows the pointer only on a node started with nothing configured.** A configured node keeps its configured source there, because that is the default a fold naming no source should use.
