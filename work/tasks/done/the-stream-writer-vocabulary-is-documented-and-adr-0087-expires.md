...
**How I restated the writer contrast (the writable-store entry).** The old contrast was a POLARITY: a stream's writer is the *oldest surviving* generation, a state store's is the *last claimant*, earliest-wins against latest-wins. That polarity is gone, so restating it as "the deployment fetches it" left a sentence that was true and taught nothing. I rewrote it as ROLE against ACT: a stream's writer is decided by what a thing DOES (whoever fetches it), so the duty is not assignable, cannot be given to something that is not fetching, and a second fold arriving changes nothing; a state store's writer is decided by an ACT and its recency, so arrival is exactly what decides and `openForWriting` takes the store from whoever held it. I then made the consequence explicit, because that is what makes the contrast worth keeping: a **demotion** exists on the store axis and has no counterpart on the stream one. Alternative considered and rejected: dropping the contrast to the `StreamWriter`-versus-`writerClaim` mechanical difference, which is true but would stop teaching why the two must not be conflated. Touches: the **writer token** entry, which carries the same distinction from the other side and is now worded to match.

**Removing the status line, given that the chain-facing container still derives the fetch (and so the append) from registration order.** This is the judgement call the task turns on, and it is the one a reviewer should check. Read literally, my task says to leave the line if "a write duty [is] still derivable from registration order", and on the browser engine it is. I removed it anyway, because what ADR-0087 retired is an ELECTION that hands the pen to a fold that is not fetching, and that shape is gone: on the receiving runtime the deployment fetches and appends, on the chain-facing one the fetcher and the writer are one object, so the invariant "a stream's writer is the thing fetching it" holds on both. Registration order there answers *which generation fetches* (ADR-0044, untouched by this ADR and strengthened by it), not *which non-fetching fold holds the pen*. The alternative was to STOP and leave the line; I rejected it because the line means "no code implements this", which is now flatly false and misleads in the more expensive direction, and because the scope boundary is not an under-build discovered by me: the blocker's builder recorded it as a deliberate decision with its cost stated, the repo carries an observation about the residue, and a task for it sits in `work/tasks/ready/` (`the-reloaded-tab-stall-is-measured-on-the-configuration-a-tab-actually-has`). The change is one line and trivially reversible if the human disagrees. What it touches: that ready task, and ADR-0044, whose follower rule is what the chain-facing derivation now rests on alone.

**I amended ADR-0087 rather than only deleting the line.** With the status gone the ADR reads as current and unqualified, and its own headline says "never by a GENERATION", which is false on the chain-facing runtime; leaving that would recreate the drift this task exists to remove, one layer up. So the ADR carries a fifth dated amendment saying what is implemented, what is deliberately out of scope and why, and naming the follow-on task BY SLUG rather than by path (the ADR's own convention, and what keeps `check:refs` green when that task lands). Alternatives: delete the line only (rejected, above), or retitle the ADR (rejected: a title is the citation handle, and thirty-odd references would rot).

**What I left alone, deliberately.** (1) `CONTEXT.md` still says registration order picks the fetching generation on the chain-facing container, because the code does; deleting it to satisfy "no entry deriving the duty from registration order" would have made the glossary false, which is the outcome the prompt ranks worst. It is framed as the FETCH derivation, with one sentence saying plainly that the retired rule was the other thing. (2) The residue itself is not written into `CONTEXT.md`: the glossary teaches the rule in force, and an open, unmeasured hypothesis belongs in the observation and the task that own it (both exist). (3) ADR-0044, 0055, 0071 and 0072 keep their bodies and their absent statuses. They are frozen records, ADR-0087 already names what it supersedes, and no earlier task in the family annotated them, so adding a `superseded in part` line is a convention call for a human rather than something to smuggle into the closing task. I noted the cost (a reader arriving from a `CONTEXT.md` ADR-0044 citation meets the retired rule with no signpost) in the observation note. (4) `Indexer.dropSuperseded` and `dropReplaced` in `packages/core/src/container.ts` still document and log a stream reap that `deleteGeneration` can no longer perform (`reapStream` defaults off, so `deletion.reaped` is always `undefined` there); that is inside this task's `packages/*/src` fence, so it is a capture note, not an edit.
