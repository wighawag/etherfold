...
**1. Reused the hot-update example's naming scheme verbatim instead of designing one.** The corrected snippet uses `app-${context.stream}-${++saves}` with `let saves = 0`, character for character the scheme the `reconfigureFromHotUpdate` example 40 lines below already uses, so the page states one convention twice rather than two. Alternatives considered: a fixed suffix (`app-${context.stream}-next`), rejected because a replaced successor's database can still be open when the next one is built, which is precisely why the neighbouring example uses a counter; and deriving the name from the processor identity, rejected because that identity does not exist when `createState` runs (ADR-0043's state-then-fold order). Touches: nothing outside this page; the prose now names `context.stream` and the counter as the two halves of the name, so the reason travels with the scheme.

**2. Forwarded the claim `{signal}` in the snapshot-boot factory but not in the `addGeneration` snippet.** The boot factory is an app's startup path and both package-README boot factories forward it, so it gets `openForWriting(store, {signal})`. The `addGeneration` snippet deliberately does not, so that it is identical to the hot-update example it sits above; criterion 2 asks those two to agree, and adding the signal to one of them would be the page teaching two shapes again. The `createState` JSDoc calls forwarding a convention rather than a guarantee, so the shorter form is incomplete, not wrong. Alternative: add `{signal}` to both, which would have meant editing the already-correct hot-update example. Touches: the hot-update example, which is why I left it alone.

**3. Corrected the root `README.md` snippet, which is outside criterion 4's literal list.** Criterion 4 names "the guide and the package READMEs"; the root README is neither, but it carries the identical defect (`createState` returning an unclaimed store), it is the project's front page, and it is the first snippet a new reader copies. Alternative considered: state its incorrectness and leave it, rejected because the criterion exists precisely because nothing in the gate catches an uncompiled snippet. It costs no changeset (the root package is private). Touches: nothing else; the wording is copied from `packages/browser/README.md`'s equivalent factory so the two agree.

**4. Corrected the guide's snapshot-boot factory as part of the sweep rather than only reporting it.** It is a second instance of defect two, found by the sweep, and criterion 4 permits correction. It is a slightly larger edit than the sweep's "check" verb implies (it adds the patience parameter and an import), so it is recorded here rather than buried. Touches: nothing outside the page.
