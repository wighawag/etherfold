...
**The container lives in `@etherfold/core`, as `ReceivingIndexer` in `receivingContainer.ts`.** In core because everything it composes is already there (the registry, the promotion/identity model, `StreamBuilder`) and because both the server and the CLI must reach it — putting it in `@etherfold/server` would make `@etherfold/core`'s own receiver depend upwards, and putting it in the CLI would leave the Worker host unable to hold generations. Named `ReceivingIndexer` rather than a new noun: `CONTEXT.md` already defines *indexer* as the named unit holding generations, and both containers ARE that; the adjective says which half of the wire (`StreamBuilder` is already "the receiving half"). Rejected: `StreamIndexer` (re-means *stream*, which is a filter+config identity, not a runtime), `GenerationContainer` as the class name (kept for the narrow PORT the receiver holds, which is what "container" means from below).

**Rules LIFTED versus rules deliberately NOT taken from the existing container.** Lifted by reuse rather than by copying: the two factories (a `Pick` of `GenerationSpec`, so the order and the keying rule have one home), and the ONE-WRITER rule via the already-shared `writerOf` — the emission appender is handed only to the writer of the stream, so a successor over a shared stream cannot double-append (ADR-0052/ADR-0044); `writesStream` reports it and nothing can set it. Deliberately NOT taken: `CanonicalGenerationNotHeldError`. On this runtime a generation's state is a table namespace and the read tier resolves the pointer to name it (ADR-0053), so the canonical generation answers with no engine at all — which is exactly the ordinary upgrade, where the host holds only the new processor. Refusing there would turn every upgrade back into the outage. Touches: `two-named-indexers-never-touch-each-others-data` and `the-cli-and-the-server-hold-generations-the-same-way`, which will build hosts on this container. Also not taken: `promotion` — creating is this task, moving the pointer is not.

**The caps are an option on the container (`caps`), and the container takes the registry PORT rather than an opened registry.** `openGenerationRegistry` takes caps as an argument, so a host handing over an opened registry would already have decided the bound inside whichever module built the port — which is how a bound nobody can see gets chosen by a substrate. Taking the port also puts the sweep at container open. Default `SERVER_GENERATION_CAPS = {maxGenerations: 4, maxStreams: 2}`: four is the incumbent + the successor + the predecessor kept so a revert stays free + one spare so a routine upgrade never meets the bound as a surprise; two streams (not four) because a generation re-folds a stream that is already stored while a STREAM is the expensive thing to re-fetch — the browser's own reasoning at the same number for the same resource. Alternative considered: no default at all, as `openGenerationRegistryOnSQL` does. Rejected because a host that must state a number before it can hold one generation makes the additive path harder than the destructive one. Touches: `ReceivingIndexerOptions.caps`, and the server options / CLI config that task 10 threads into it.

**The generation is registered at container OPEN, not on the first batch.** So a cap refuses at start-up, in an operator's boot log, naming what to delete — rather than as a `500` on somebody's ingest — and the record exists with the widest possible margin before anything writes the stream (the registry's own ordering rule). The receiver still resolves-or-creates on every cursor read (memoised on the container), which covers a version hash that moves after open and keeps `StreamBuilder` honest with a container it did not build. Alternative considered: register lazily at the `clear()` branch only; rejected because that branch is unreachable once state is per generation, so the generation would never be registered on the happy path.

**`entityProcessorVersionHash` is a new export on `@etherfold/processor-entities` (a second package).** ADR-0053 asserts the namespace is derivable before the processor exists; nothing exposed the value it is derivable from, so a host would have had to re-spell `${version}-${simple_hash({entities, config})}`. This is not the thing ADR-0043 rejected (a caller *declaring* a hash that can silently disagree): `getVersionHash()` now returns exactly this call, so the two cannot diverge. Touches: any host naming a generation's table namespace, which is task 10's job.

**Scope held at the container; the commands are NOT rewired.** `prepareIndexing`, `indexCommand` and `folding.ts` are untouched, because `the-cli-and-the-server-hold-generations-the-same-way` owns that move (and is blocked on two tasks that also rework `folding.ts`). The host assembly this container needs is exercised in `packages/cli/test/aChangedContextCreatesASuccessor.test.ts` — real SQL registry port, real namespaced stores, one handle — so the container is proved over the real substrate and task 10 has the shape it will move into `folding.ts`.

**No new ADR.** Every choice above sits inside a decision already recorded (ADR-0043, ADR-0044, ADR-0052, ADR-0053) or is cheap to reverse (the caps default, the container's name, where registration happens). The one that came closest to the ADR gate — not lifting `CanonicalGenerationNotHeldError` — is already implied by ADR-0053's "a read tier must resolve the canonical pointer before it can name a state table", and is a refusal that could be added later without moving any bytes.
