...
**1. How `commit` stays atomic: a GUARDED BATCH carrying a REVISION TOKEN (ADR-0054, written).** A commit reads the records, the pointer and a `revision` token in one batch; every statement it then writes is conditional on that token (`COALESCE((SELECT revision ...), '') = ?`); the last write swaps the token for a fresh unique one; and a final `SELECT` *inside the same batch* reports the token the row now holds. Ours means we won and every guarded statement applied; anything else means our whole batch applied to nothing, and we re-read, re-run the decision function and retry. Alternatives weighed: pushing the decision into SQL (rejected: the decision is not one predicate, it resolves duplicates, applies two caps and refuses naming every deletable generation and stream, so SQL would fork the model this task consumes); a monotonic integer revision (rejected: a loser reads `expected + 1`, which is the winner's value *and* the number the loser meant to write, so a win is indistinguishable from a loss, and `remote-sql` reports no affected-row count); a lock row (rejected: a crashed holder wedges the registry and needs a second timeout mechanism); in-process serialisation (rejected: the target host is serverless, so two isolates are two writers). Touches: any future substrate over this seam, and the port contract, since `plan` must remain a re-runnable pure decision (it already is).

**2. The revision guard lives ON the canonical-pointer row, not a table of its own.** Every commit already writes that row, so the guarantee costs no extra row and no extra statement, and "absent canonical means leave it where it is" falls out of binding `NULL` and `COALESCE`ing. The cost is one column on `_generation_pointer` that is not about the pointer; said out loud in the schema, the module and the ADR. Table names: `_generations` and `_generation_pointer`.

**3. Writer succession: the rule is DERIVED, and only its durable half is built here.** `writerOf` is the oldest surviving registered generation on a stream, computed from the records, so succession is atomic with the delete *because it is stored nowhere*: no writer column, no second write for a crash to land between. That is the whole of what this task's seam can honestly enforce. The ENGINE half named in the task body (swapping a successor's `readOnlyStream` for the real writable `ExistingStream`, and moving the wire context's receiver) is **not built here and cannot be**: on the server tier an `IndexerRegistryEntry` holds exactly one live wire context and there is no generation container above `StreamBuilder`, which are `one-registry-entry-holds-several-live-wire-contexts` and `a-changed-context-creates-a-successor-instead-of-clearing`. I did not fake it with a test that cannot fail (today's receiver is generation-blind, so "a batch still appends after a delete" passes for the wrong reason). The ADR-0044 amendment states the rule, states that succession is atomic because it is stored nowhere, and names those two tasks as the place the engine half lands, so they inherit it rather than deciding one of their own. Touches those two tasks and `the-rebuild-replays-the-local-stream-in-bounded-chunks`, which already cites this rule.

**4. ADR-0046 is deliberately NOT reopened.** Drop-on-promotion still DECLINES to drop a writer another held generation follows: an automatic drop taken inside an advance cycle is a different act from an operator deleting a generation on purpose, and I changed no container behaviour in `@etherfold/core`. The amendment says so explicitly, because "the writer succeeds on a delete" and "the automatic drop declines" otherwise read as two rules in tension.

**5. `writerOf` went into `@etherfold/core`'s registry (the rules), not into the SQL substrate.** The rule is substrate-neutral and is now asserted over memory and SQL; putting it in the server package would fork it across three substrates, which is the duplication the rules-over-a-port shape exists to prevent. It adds one method to the `GenerationRegistry` type (nothing else implements that type).

**6. A new refusal: `GenerationCommitContentionError`, after `MAX_COMMIT_ATTEMPTS = 8` lost guards.** Nothing is written and the caller may retry; the bound exists so sustained contention surfaces as a refusal naming it rather than as an unbounded loop that is indistinguishable from a hang. It is exported from `@etherfold/server`.

**7. `openGenerationRegistryOnSQL` DEFAULTS no caps: `caps` is required.** The browser has `BROWSER_GENERATION_CAPS` (two of each) because a tab keeps the previous generation only until promotion; a server or a CLI should be far more generous and the number is a deployment's statement, so a substrate module picking one would be an invisible user-visible default. Touches `a-changed-context-creates-a-successor-instead-of-clearing`, which the task names as the owner of the cap VALUES on this runtime.

**8. Consequence worth flagging to the next tasks: the first registry open on a PRE-GENERATION database sweeps its stored emission stream.** `listStreamDigests` answers from `_emissions`, so a stream written by today's `run` / `index` (which register no generation) is claimed by nobody and the sweep on open drops it. That is the registry's designed behaviour rather than a new one, and its documented cost is a re-fetch rather than a hole, but it is real for `a-changed-context-creates-a-successor-instead-of-clearing` (which opens the registry before it can create anything) and for `the-rebuild-replays-the-local-stream-in-bounded-chunks`. Documented on `listStreamDigests`; no migration written, per the repo's "a one-time migration between two unreleased formats has no beneficiary" convention.

**9. `CONTEXT.md`'s `follower` entry was updated** so the glossary's writer sentence is the restated rule rather than the superseded half of it.
