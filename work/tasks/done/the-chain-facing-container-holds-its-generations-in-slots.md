...
**The rule is SHARED as one pure function; the ACTION is REPEATED in each container.** `displacedBySuccessor(arriving, generations, slots, heldHere)` lives in `generation/registry.ts` beside `slotHolding` and `unslottedGenerations`, which are already exactly this kind of shared pure rule, and both containers call it. I shared it rather than repeating it because it carries the safety clause — a generation any OTHER slot names is untouchable, which covers the incumbent and the revert target together — and two copies of a safety predicate is how a revert target gets deleted by the twin nobody was reading (ADR-0071's "one rule has one home"). I did NOT hoist the slot LOGIC: whether a drop strands a follower, and what stopping a fold means, are repeated and worded alike, because a fold is an engine on one side and a receiver on the other, and a shared base parameterised over storage, caps and what a fold IS is the options-growing abstraction ADR-0071's closing section already refused. Alternatives considered: repeat everything (rejected — the safety clause would have two homes), or share a `SlotReplacer` collaborator (rejected — five callbacks to save sixty lines). Touches: `ReceivingIndexer.replaceTheSuccessor` was refactored onto the shared function, and `displacedBySuccessor` is a new public export of `@etherfold/core`.

**What a cap of TWO means under THREE slots: the tab holds `canonical` + `successor` OR `canonical` + `predecessor`, never all three, and the third registration is REFUSED rather than dropping the revert target.** Three slots occupied at once is three generations and `BROWSER_GENERATION_CAPS` is two, so after a promotion (which assigns `predecessor` and clears `successor`) a further save meets `maxGenerations`. I did not soften that, and specifically did not let a replacement reach `predecessor` to make room: that would be an eviction, and no policy can know which generation was being kept, while the re-index it costs may not be available at all from a public node. This is NOT a new wall — `promotion.test.ts`'s existing "a superseded generation is never EVICTED by a cap" pins the identical refusal against the pre-slot code, and the acceptance criteria say the caps are unchanged — and what slots DO fix is the churn case, which is what a cap was always the wrong instrument against: repeated reconfiguration without an intervening promotion is now bounded at two forever. It is asserted as a test ("REFUSES a third generation rather than dropping the revert target") so the trade is visible rather than silent, and recorded as an amendment in ADR-0084 beside the consequence that argues the caps should stay. Alternative considered and deliberately NOT taken: raise `BROWSER_GENERATION_CAPS.maxGenerations` to three so all three slots are expressible at once — that is a storage decision of its own and the acceptance criteria forbid touching the caps. Touches: `an-hmr-update-reconfigures-the-tab-it-is-running-in` (an HMR loop that promotes between saves will meet this), and any future task that revisits the browser caps.

**A move onto a generation NO slot names now drops nothing, where `everCanonical` could have classified it as a promotion.** Deleting the flag means "was this a promotion" is `successor` naming the target, so an operator promoting some other registered generation is treated as a move that supersedes nothing. Strictly more conservative in the destructive direction, and it makes the chain-facing container agree with the receiving one, which recorded the same choice when it landed.

**A host opened with several generation specs now puts every spec after the first into `successor`, so the ones between are replaced and dropped as they are added.** That follows from registering with a slot at all and is the correct reading of "successor holds at most one"; the only shape that behaves differently from before is a container opened with three or more specs, which nothing in this repo does outside tests. Called out in the changeset because it is the one caller-visible behaviour change.
