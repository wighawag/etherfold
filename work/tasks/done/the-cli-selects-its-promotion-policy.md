...
**1. `--drop-on-promotion` is exposed too, not just the policy.** Acceptance criterion 4 requires the `immediate`+dropping combination to be refused as a configuration error, and with only the policy exposed `dropOnPromotion` is always `false`, so that combination is unreachable and the criterion is vacuous. The alternative was to expose only the policy and drop criterion 4; I rejected it because the container's own refusal message says "use `on-catch-up` with `dropOnPromotion`", which is unreadable advice to an operator who cannot set it. Touches: a new user-visible flag that can destroy the revert path (story 4), default `false` and argued by ADR-0046, so the unsafe direction stays opt-in.

**2. Only `run` owns the input; `build`, `index`, `fetch` and `serve` REFUSE it by name.** The task says "every command that holds generations accepts it", but the policy only ever speaks when a successor is added to a *running* container, and `container.add` is reached from exactly one place in the tree: `reconfigure.ts`, wired only by `run` (`build` stops at the tip and never calls `rebuildMore`; `index` wires no reconfigure route and schedules no rebuild; `serve` folds nothing). Accepting it elsewhere would ship the accepted-and-ignored flag into the one module whose header forbids it. Alternative considered: accept on `index` and `build` anyway, for symmetry. Rejected on ADR-0048's own recorded asymmetry — refusal→optional is additive later, removing an accepted flag is breaking. Touches: four commands' ownership rows and refusal messages, and whichever future task gives `index` a reconfigure (it will need to flip one cell).

**3. A new environment variable, `PROMOTION_POLICY`.** Criterion 1 requires selection "by a flag or by the environment". ADR-0048's *rule* puts it there (the environment carries what varies between deployments of one image; the same processor image iterated locally versus shipped is exactly that), though the ADR's prose enumerates a list of six that was already stale before this change (`PRUNE_INTERVAL`). I updated the stale count/claim in `config.ts`'s own doc comment for `INPUTS`, and did not edit the ADR. Touches: the deployment contract of the `etherfold` image.

**4. `--drop-on-promotion` gets no environment variable.** It is a boolean, and this module has never had a truthiness spelling for the environment — the only other boolean, `--no-auto-setup`, is flag-only for that reason. Inventing `DROP_ON_PROMOTION=true|1|yes` would add a parsing convention for one input. Alternative: give it one and pick a convention. Reversible if a containerised deployment needs it.

**5. The policy is reported on `/status` as a top-level `promotion` field, not inside the `cursor` envelope.** The `cursor` envelope is about progress, and a promotion policy is not progress; putting it there would re-mean that field. Instead it mirrors `getFetcherLimits` exactly — an injected reporter for a deployment-level fact only the process holding the thing can answer, typed rather than opaque (it is `@etherfold/core`'s `UsedPromotionConfig`, not a host's private vocabulary), degrading to a reason rather than an omission. Alternative considered: `GET /{indexer}/admin/canonical-generation`, which is the operator's pointer-move route; rejected because the policy is a process-level configuration rather than a fact about one named indexer, and `/status` is the page an operator already watches a rebuild on. Touches: `@etherfold/server`'s `ServerOptions` and `platforms/nodejs`'s `StartOptions` (both additive and optional).
