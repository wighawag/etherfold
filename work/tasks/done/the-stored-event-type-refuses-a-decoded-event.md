...
- **`reparse` takes an explicit union (`readonly (LogEvent<ABI> | StoredLogEvent)[]`) rather than the supertype (`readonly BaseLogEvent[]`).** Chosen because the union NAMES the two things a read may be handed, while annotating the parameter with the supertype would accept literally any event including ones nothing decoded, which is the exact "enforces nothing" property the spec refuses `BaseLogEvent` for elsewhere; a reader of the signature would then have no way to tell that a stored stream is what this is for. Alternatives considered: the supertype (rejected, above), and making `reparse` generic over `Extra` (rejected as scope creep — every caller runs at `Extra = undefined` and no consumer asked for it). What it touches: `the-stream-seam-takes-only-the-stored-event`, which narrows `StreamFetcher` to hand back stored events. That task needs no further change here, since `readonly StoredLogEvent[]` already satisfies this parameter; if it ever decides reads should be stored-only, dropping the `LogEvent<ABI>` arm is a one-line contraction, but I have NOT done it, because reads still legitimately carry decoded events off segments written before this change.
- **The internal destructure now asserts `BaseLogEvent & {args?: unknown; eventName?: unknown; decodeError?: unknown}` instead of `LogEvent<ABI> & {...}`.** A union input cannot be asserted to one member without claiming something false about the other. The supertype is the honest common shape here, and it is an internal assertion inside the one function that immediately drops those three keys, not a seam annotation — the "never annotate a seam with the supertype" rule is untouched. Alternative considered: destructuring without any assertion (it does typecheck, both arms being assignable), rejected because the explicit optional-`unknown` shape is what documents that the three keys are being deliberately discarded.
- **The known widening hole is asserted as a COMPILING line in the type test, not only described in prose.** `storesOne(widened)` (a `BaseLogEvent`) sits in the refusal function with no `@ts-expect-error`, commented as the hole. This makes the limit executable knowledge rather than a paragraph nobody re-reads. The cost, stated at the site: if a later change ever closes the hole, that line goes red and the comment must be rewritten. Alternative considered: documenting it in the docstring only (rejected — a hole that is only prose is exactly how the supertype gets reached for again).
- **No new ADR.** The decision this task carries (a distinct exclusion-form type rather than reusing the supertype, and `EmittedLog` surviving beside it) was made by the human in the task body and by the spec, and its durable WHY now lives in the exported type's docstring plus the changeset. Adding an ADR would be a third copy of a decision already recorded in two durable places, and the constraint it operates under (ADR-0034) already exists.
