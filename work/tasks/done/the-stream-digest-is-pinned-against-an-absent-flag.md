...
**A `patch` changeset for a test-only change, rather than `changeset add --empty` or none.** The gate runs `pnpm changeset status --since=main`, and I verified empirically that once a file under `packages/core/` is *tracked* it errors with "Some packages have been changed but no changesets were found" — so "none" is a guaranteed red gate at land time. Between the remaining two: an empty changeset is the semantically exact instrument, but there is no precedent for one in the 96 files in `.changeset/`, so it would introduce a new repo convention as a side effect of an informative task. A patch bump costs nothing today ("NOTHING IS PUBLISHED YET") and puts the answer into the release notes of the release that will also carry the deletion, which is where a reader of that breaking change will look for it. Touches: `@etherfold/core`'s next version number, and the changeset convention in `CONTEXT.md` (`## Conventions`).

**Both halves in ONE new self-contained test file, rather than extending `streamIdentity.test.ts` and `streamLoad.test.ts`.** The task declares itself a deliberate orphan that "should be dropped rather than defended if it ever costs more than that", and a single file is the droppable unit; splitting it across two established files would make dropping it a surgical edit of tests that are not orphans. It also keeps the existing assertions untouched, satisfying the "not duplicated" criterion by construction. Alternative considered: putting the literal beside the existing literal in `streamIdentity.test.ts` (where a reviewer might look first) — mitigated by the new file's header naming ADR-0073 and pointing at `streamIdentity.test.ts` for the assertions it deliberately does not repeat. Touches: nothing outside the new file.

**A digest-ADDRESSED memory keeper, defined in the test file, instead of the shared `memoryStream`.** `packages/core/test/utils/streamCacheWorld.ts`'s `memoryStream` holds exactly one stream and serves it whatever it is asked for, so a moved digest reads back the same events and "no fork" is unassertable through it. The new wrapper keys one `memoryStream` per digest and takes the resolved config through `setStreamConfig`, mirroring `keepStreamOnIndexedDB`. I deliberately did **not** modify `streamCacheWorld.ts` (e.g. adding a fifth positional stream-config parameter to `makeIndexer`), because that helper is shared by several suites and widening it for one informative test is a cost the other suites pay; the new file composes it instead. Touches: `packages/core/test/utils/streamCacheWorld.ts` is unchanged, so no other core suite is affected.
