...
- [ ] An ORDERED location list is honoured and failover walks it, so a caller can put a rolling remote first and a BUILD-EMBEDDED artifact last and still start when the remote is unreachable (the shape the reference deployment uses).
