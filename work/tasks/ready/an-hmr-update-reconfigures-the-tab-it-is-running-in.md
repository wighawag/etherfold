...
- [ ] A deployment built WITHOUT an HMR-capable bundler is unaffected and pays nothing: no dev-only import survives into a production bundle.
...
The decision most likely to be got wrong is how the tab learns. `import.meta.hot` is bundler-specific and must not leak into a production bundle or into a consumer that does not use a bundler at all. Decide whether the package EXPORTS a function the developer calls from their own `import.meta.hot.accept(...)` handler, or whether it subscribes itself, and weigh them honestly: subscribing is fewer steps for the developer and puts a dev-only global inside a published library, while exporting a function keeps the library clean and asks for three lines in the app. State which you chose and what it costs. This is the whole design content of the task.
...
RECORD non-obvious in-scope decisions you make while building, in a `## Decisions` block at the end of your FINAL REPORT. Whether the library subscribes to HMR or exports a function for the app to call, and what the tab reports for each of the three outcomes, are both such decisions. Do not write the done record, the commit message or the PR body yourself, and do not open an observation note for a decision you made.
