...
- [ ] `version`, `assertProcessorVersion`, `getVersionHash()` and `getCodeFingerprint()` are gone from the published surface and from the tree.
- [ ] `utils/fingerprint.ts` SURVIVES, holding `processorCodeFingerprint` in its new role. What goes is `getCodeFingerprint()` from the processor seam, which is the fingerprint as a SECOND OPINION beside a declared identity; what stays is the derivation that IS an identity where no bytes exist. Its own tests (`packages/core/test/processorFingerprint.test.ts` and its fixtures) stay with it, because they test the surviving derivation rather than the drift report, and removing them under the criterion below would delete live coverage. (This criterion previously required the file to be "gone from the tree", which contradicted this task's own Prompt and "What to build"; corrected 2026-09-18.)
