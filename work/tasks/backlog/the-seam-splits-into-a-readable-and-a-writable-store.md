...
**The split is at the INTERFACE, and the concrete backend classes are unchanged.** That is the load-bearing decision and it is what keeps this task small: the mutating call sites in this repository are overwhelmingly TESTS that construct a backend concretely (`new VersionedStateStore(...)`, 156 call sites in the SQLite suite alone) and then mutate it. Those hold the CLASS type, not the seam type, so they are untouched by a change to the seam and must stay untouched. Only code that holds a `StateStore`-typed value migrates, and that is roughly sixteen sites across five packages.
...
type StateStore = { /* getCurrent, getAsOf, listCurrent, listAsOf, readCursor, capabilities, migrate */ };
...
- [ ] `openForWriting` and `openForReading` exist with ONE export site and ONE signature, stated in the module that owns the seam, so five migrating callers cannot each invent a call shape.
...
Precedent: ADR-0044 makes the one-writer rule STRUCTURAL for streams, "the writer is handed the keeper, every follower is handed a read-only stream view". This is that move for state. The one existing case that swallows writes instead, `readOnlyStream`, did so for a documented reason that does not apply here.
