---
title: 'The cf-worker platform opens its store for writing'
slug: the-cf-worker-platform-opens-its-store-for-writing
...
Move `platforms/cf-worker` onto `openForWriting`, so it holds a store whose type says it writes rather than one that merely happens to.
...
On D1 the atomic unit is exactly one `batch`, so confirm the guard rides inside the batch the block already writes rather than becoming a second round trip.

## Acceptance criteria

- `platforms/cf-worker` constructs its store through `openForWriting` and holds the writing shape.
