---
title: 'The processor layer holds a writable store'
slug: the-processor-layer-holds-a-writable-store
...
Move `@etherfold/processor-entities` and `@etherfold/processor-sqlite` onto the writable shape, so what writes is typed as writing and what only reads is typed as reading.
...
The sites: `packages/processor-entities/src/apply.ts` (six mutating calls, the most in any consumer), `EntityEventProcessor.ts` (the `store` field and the `prune` pass-through), `view.ts`, `snapshot.ts`, and `packages/processor-sqlite/src/VersionedStateEventProcessor.ts`.

This layer RECEIVES a store rather than constructing one, except `VersionedStateEventProcessor`, which builds a `VersionedStateStore` from a `RemoteSQL`. So the obligation is mostly to move PARAMETER and FIELD types to the writable shape and narrow anything that only reads. It drives every mutation in the system, so it is the batch most likely to reveal a gap in the writable shape: do it first among the batches if you have a choice.
