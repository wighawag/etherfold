...
**A defect in production code, surfaced by the port.** An entity column named `index` passes `@etherfold/state-store-sqlite`'s identifier validation (matches the regex, no `_` prefix) and then fails at `migrate()` with `near "index": syntax error`, because identifiers are interpolated into DDL unquoted and `INDEX` is a SQL keyword. The in-memory and IndexedDB backends accept the same declaration. One declaration, valid on one backend and fatal on another, failing at migration rather than at declaration. Not fixed here (this spike changes no production code beyond capture/replay); the port renamed to `playerIndex`. Captured with a fix shape in `work/tasks/done/entity-identifier-sql-keyword.md`.

**Also absent and load-bearing:** the spec says a retention window is enforced by pruning, and `@etherfold/state-store-sqlite` has no pruning at all, so every backend it ships today is effectively `unbounded`. The footprint numbers above used pruning written inside the spike. **[The sentence about the CODE is SUPERSEDED; amended 2026-09-10, below. It stands as written because it records what was true on 2026-08-22, and the measurements beside it are unaffected.]**

> **Amended 2026-09-10** by `the-prune-claims-in-the-docs-match-the-code`. This corrects a claim about CODE, not evidence: no measurement, table or conclusion in this finding changes. The sentence above bundled two separate things, a BACKEND that cannot prune and a DEPLOYMENT that therefore keeps everything, and they have come apart since it was written: the backend half is now false outright and the deployment half is true only of some hosts.
>
> **Both shipped backends now implement pruning.** `VersionedStateStore.prune` (`packages/state-store-sqlite/src/store.ts`) takes a budget, selects and deletes the versions closed at or below the retention floor in statements bounded by `maxRowsPerStatement`, logs what it dropped and reports whether the pass finished. `IndexedDBStateStore.prune` (`packages/state-store-indexeddb/src/store.ts`) does the same oldest-close-first as a range scan over the `upper` index, rather than the full scan the spike's prototype used (the 6.3 s number above). Neither touches a live version or a block record, and neither `VACUUM`s, so the SQLite footprint row above (prune plus `VACUUM`, 1.1 s, 7.05 MB to 3.82 MB) still describes the spike's prune: the shipped one frees pages for reuse without shrinking the file.
>
> **The half that outlived the code is that no HOST called it, and even that has moved.** When this was written nothing in the repository scheduled a prune, so a configured window was advisory in every deployment. Today the browser indexing loop (`packages/browser/src/IndexerState.ts`) and the CLI's `run` and `build` (`packages/cli/src/pruning.ts`, driven from `packages/cli/src/index.ts`) schedule a bounded pass per cycle, while a bespoke host still has to schedule its own, which is what `d1PruneBudget` sizes for a Worker (`platforms/cf-worker/src/d1.ts`).
>
> **Further amended 2026-09-11:** `etherfold index` schedules one too, on a timer it owns (`packages/cli/src/indexCommand.ts`), so every command in this repository that accepts `--retention` now enforces both halves of it. When the amendment above was written it did not, which is why the sentence named it.
>
> So the honest reading of the superseded sentence is: "no backend prunes" is false, and "a configured window reclaims nothing" is now true only of a deployment whose host schedules nothing.
