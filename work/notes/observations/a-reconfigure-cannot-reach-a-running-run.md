...
- **Auto-promotion is real, and it is the DEFAULT.** `promotion.ts` states "`on-catch-up` is the DEFAULT EVERYWHERE", with the reasoning that there is "deliberately no per-runtime and no per-environment default, because the axis that would select one is NOT DETECTABLE". So the assumed dev behaviour (promote when the successor is level) is the default in every environment, and the CLI exposes no flag to change it. CORRECTED 2026-09-14, because this bullet read as a claim about the CLI and is false as one: the policy speaks only for a fold added to a container that is already OPEN (`applyPolicyTo` returns while `opened` is false, and `open()` adds the configured fold before setting it), and nothing under `packages/cli/src` calls `container.add`. On a restart, therefore, the pointer does NOT move on its own under any of the three values, and the incumbent stays canonical for ever. Auto-promotion is real for a container that is HANDED a successor, which is what the reconfigure endpoint will do, and inert on every CLI deployment until then.
- **The incumbent keeps ANSWERING throughout**, and for the common reconfigure it also keeps ADVANCING: a processor-only change is a follower on the same stream (ADR-0044, it "fetches nothing at all"), so the incumbent stays the stream's writer and stays current off the wire while the successor re-folds, after which the pointer moves on its own PROVIDED the successor was added to a running container rather than arriving with a restart (see the correction above).
