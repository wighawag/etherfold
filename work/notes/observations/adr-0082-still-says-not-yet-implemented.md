...
**RESOLVED.** `createindexerstate-becomes-the-main-thread-host` (PR #141) removed ADR-0082's `status: accepted, not yet implemented` frontmatter and the `NOT YET BUILT` marker on `CONTEXT.md`'s **indexer host** entry, and brought the **tx inclusion** and **processor kind** entries into line with what shipped.
