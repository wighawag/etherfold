...
- **The COUNT of generations under churn**, which is a different axis from whether any of them is being fed. A rapid succession of changes used to accumulate one generation per save until a cap refused; that is now bounded by dropping a successor a newer one has abandoned (`a-successor-that-was-never-canonical-is-superseded`). This note is about a generation that IS held and is not being fed, however many of them there are.
