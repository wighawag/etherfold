...
Fixed in all four places, which was one more than this note found: ADR-0073's "PERMANENT machinery" paragraph (with a dated correction note), ADR-0002's block-timestamp bullet, the `TimestamplessLogError` docstring AND its operator-facing message in `packages/core/src/errors.ts`, and `blockPointer`'s fold-time refusal message in `packages/processor-entities/src/stream.ts`. The two error messages are the ones that mattered most, since they are what an operator reads while trying to fix this. All now name the surviving mechanism (a cache entry replaying an absence recorded from a forked pre-spec remote) rather than the one the 0.20.0 release note contradicts.
