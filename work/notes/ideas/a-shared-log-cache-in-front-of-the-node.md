...
## The alignment work, which is the only part with a home already

One piece of this arrived from the perf spec that was split (`work/specs/dropped/the-indexing-loop-is-round-trip-bound.md`, story 22) and belongs here, because it only pays off if this idea is built.

The fetcher's ranges are arbitrary today. Making them ALIGNED is the precondition for anything above level 1, and the pleasing part is that it composes with the adaptive loop rather than replacing it: if bucket sizes are POWERS OF TWO, then halving an aligned bucket yields two aligned buckets, so the existing halve-on-refusal behaviour already produces well-formed buckets at the next size down, and growing after a success is a doubling that stays aligned as long as it starts on a boundary.

What it costs, stated so it is weighed rather than discovered: the fetcher can no longer ask for exactly the range it wants. Resuming from an arbitrary cursor gives one partial bucket at the start, catching up to an arbitrary tip gives one at the end, and a partial bucket is not cacheable. It also gives up some sizing precision, since the ideal range is rarely a power of two. That is a real regression to the fetcher for a benefit that is zero until a cache exists, which is exactly why it lives in this note and not in `the-fetcher-reads-the-hints-providers-already-send`.

## When to actually do it

**Not before the round-trip work is measured.** This cache does nothing at all for the FIRST client, and if it turns out that the fold's own round trips dominate a backfill rather than the log fetching (which `work/specs/proposed/measure-the-indexing-loop-before-optimising-it.md` exists to find out), then a log cache is an optimisation of the smaller half. Level 1 is cheap enough to do on a hunch; levels 2 and 3, and the alignment work above, should wait for a number.
