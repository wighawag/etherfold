...
The observation this spec came from has been discharged, its signal now carried by `an-endpoint-triggers-a-reconfigure-in-a-running-process`. The negative evidence it recorded, restated here so this spec stands on its own: nothing in the CLI watched a file, no route added a generation, and the only signals handled were SIGINT and SIGTERM, so a changed processor reached a running deployment only by RESTARTING it. The endpoint shape follows from that, with whatever notices the change left deliberately outside the process. The churn constraints it was raised beside are now settled in `a-successor-that-was-never-canonical-is-superseded`, and are restated here so this spec stands on its own. A cap REFUSES at its bound and never evicts, which is the right mechanism against slow accumulation and the wrong one against churn, so an abandoned successor is DROPPED when a newer one takes its role rather than the bound being raised. A processor that does not compile is the normal case in a dev loop and must leave the running deployment exactly as it was, failing BEFORE anything is registered rather than unwinding afterwards. And debouncing belongs to the WATCHER, outside the process, though note that even a correctly debounced watcher still produces one generation per successful build, so debouncing alone settles none of the above.
