...
- **The promotion trigger's inability to read an unheld generation's cursor** (the observation `the-promotion-trigger-cannot-be-evaluated-with-no-held-incumbent`; since landed, per ADR-0084 amendment of 2026-09-19). It looks like the same missing-engine fact and it is not: the trigger needs a NUMBER, which is a row addressed by an identity the registry already holds. It is fixed by a cursor seam beside `dropState` and must NOT wait for this spec.
