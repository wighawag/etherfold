...
1. **Who runs the bundler?** Today `etherfold run` resolves a configuration and `import`s a module from a path. If a self-contained bundle is REQUIRED, either the CLI bundles the entry point itself (no change to how an author works, a bundler dependency in the CLI, and a build step on every start), or the author supplies a pre-built artifact (no bundler in the CLI, an explicit build step in their workflow, and a worse first-run experience). ADR-0085 assumes the artifact already exists because it is being PUSHED; this spec needs it on the ordinary local path too, where nobody has built one.
2. **Does the `import` path survive at all?** If the CLI bundles on start, `import` becomes an implementation detail and every generation gets an artifact for free. If it does not, a deployment that points at a module path has no artifact to retain, and generations split into resumable and frozen, which question 3 says we do not want.
3. **What happens to a generation registered before its artifact exists?** Nothing is published (`CONTEXT.md`), so there is no migration, but there IS the case of a deployment whose author has not yet adopted whatever question 1 decides. Refuse to register, or register a frozen generation and say so? Refusing is consistent with this repo's habits; registering-and-saying-so keeps the upgrade path open.
4. **Is the artifact's hash the `version`, or beside it?** ADR-0085 says "the hash of the received bytes serving as the processor's `version`". On the local path an author still DECLARES a `version`. Two sources of identity for one processor is exactly the confusion `a-reload-that-changed-nothing-reports-processor-drift` exists to report on, so this must be settled rather than left to coincide.
