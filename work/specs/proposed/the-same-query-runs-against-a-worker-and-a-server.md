...
- **Two stories moved here from the worker spec when that spec was tasked (2026-09-11).** It launched carrying "query the worker with the same document I would send to a server" and "the query executor's serialisation is the one the query spec pins", which describe the executor ON the port rather than the port itself, and which its own Out of Scope already assigned here. They are this spec's stories 1, 5 and 13 said a second time, so nothing was added: the record is here because a reader comparing the two specs will notice the worker spec now stops at the port, and the reason is that tasking it atomically was impossible while two of its stories depended on a `QueryExecutor` that does not exist and that this spec defines.
