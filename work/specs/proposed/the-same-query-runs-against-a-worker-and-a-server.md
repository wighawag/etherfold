...
3. **Is multiEntry with ARRAY subkeys reliable on all three engines?** It is spec-legal, verified against the text: "convert a value to a multiEntry key" converts each item with "convert a value to a key", which accepts an Array exotic object and rejects only cycles, and "store a record into an object store" adds one index record per subkey. Engine behaviour in this corner has historically diverged. **This gates rung 2 and is the one thing here that needs a SPIKE**, on the existing three-engine Playwright harness.
4. **What bounds rung 1, and in what unit?** Rows scanned, rows returned, or elapsed time. Rows scanned is the honest one (it is what actually grows), elapsed time is what a user feels, and the two disagree exactly when it matters.
5. **In the browser, is `block:` combined with `where` refused, or served by scanning versions?** The tip path scans 4,072 live rows on the real measured workload; the as-of path scans versions, of which the same workload has 29,393. Refusing is honest and asymmetric with the server; scanning is uniform and unbounded.
6. **Where does the accessor seam live?** `@etherfold/state-store` (neutral, beside `createReadSurface`, and then the seam has a member no backend can implement without a planner) or its own package that both backends and the schema depend on.
...
- **The multiEntry spike**, on the existing three-engine Playwright harness, before rung 2 is tasked: array subkeys, key ordering including binary keys for a sortable big number, and the write-path cost of maintaining the index. ADR-0024's own consequences note that the shipped `lower` and `upper` indexes were added AFTER the 45.6 ms/block measurement and never re-measured, so a third index lands on an already-unmeasured regression and must be measured rather than assumed.
