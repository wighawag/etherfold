...
The first of those two commands is a BUNDLE step, and it is not packaging: a processor's identity is the sha256 of its bundle ([ADR-0086](docs/adr/0086-a-processors-identity-is-derived-from-its-code-and-never-declared.md)), so `-p` is pointed at one file whose bytes name the fold. [Producing the processor bundle](packages/cli/README.md#producing-the-processor-bundle) is the one command that produces one, and why `--minify` is part of the identity rather than a size preference.

To keep that database CURRENT and answer over it, swap one word: `etherfold run` is the same assembly that follows the chain instead of stopping at the tip, and serves on the port you name -- `/status` (health, reorg counters, and a cursor that advances) plus the read surface of the named indexer it folds: `/{indexer}/feed`, `/{indexer}/canonical`, `/{indexer}/state-moved` and the operator's `/{indexer}/admin/canonical-generation` (which generation answers reads, which slot holds each of the others, and the move that promotes or reverts) beside `/{indexer}/admin/reclaim-generations` (take back the disk a generation no slot names is holding). It accepts no pushes (it fetches the chain for itself, so a remote sender would be a second writer) and says so rather than going quiet. One process, no split to think about.
