...
To keep that database CURRENT and answer over it, swap one word: `etherfold run` is the same assembly that follows the chain instead of stopping at the tip, and serves on the port you name -- `/status` (health, reorg counters, and a cursor that advances) plus the read surface of the named indexer it folds: `/{indexer}/feed`, `/{indexer}/canonical`, `/{indexer}/state-moved` and the operator's `/{indexer}/admin/canonical-generation`. It accepts no pushes (it fetches the chain for itself, so a remote sender would be a second writer) and says so rather than going quiet. One process, no split to think about.
