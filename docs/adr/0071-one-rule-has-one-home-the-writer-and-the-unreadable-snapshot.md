...
> **AMENDED 2026-09-17 (`the-chain-facing-container-holds-its-generations-in-slots`): the set question is SUPERSEDED, and `Indexer.add` now derives `follows` from `writerOf` after all.** This section's REJECTION of the `writerOf` form was right when it was written and both of its stated preconditions have since been met, so what is left is a rule with two homes again. The tie is gone: ADR-0072 made `createdAt` strictly increasing within a registry, which is precisely the "stable in REGISTRATION order" this section names as the open work, so the same-millisecond case that produced two writers cannot occur. And the RECONCILIATION this section names beside it turns out not to be required for the derivation itself: `readOnlyStream` being baked into the engine's config at construction only matters when the writer CHANGES while a fold is held, which is a different case, still unbuilt, and untouched here.
>
> **What forced it is that "the two now give the same answer" was FALSE**, and the code carried that claim as a comment. It holds only while the record being added is NEW -- then it sorts LAST and `writerOf` can never name it while others exist. A registration that RESOLVES to a record already in the registry keeps its original `createdAt` and sorts FIRST, and that is exactly what a page RELOAD does: a tab re-opening on canonical A with a leftover successor B still registered found B "already on this stream" and built its own CANONICAL generation as a FOLLOWER of a stream nothing writes. `writerOf` names A correctly in that case; the set question does not. Before durable slots the path was mostly unreachable, because a reload with a changed fold registered a THIRD generation and the cap refused it loudly, so making the reload OPEN is what turned a refusal into a silent stall and made the divergence worth paying for. The failure is pinned by the reload test in `@etherfold/browser` (`packages/browser/test/aTabHoldsItsGenerationsInSlots.test.ts`), which asserts that the reloaded container still FETCHES and not merely that its slots read right; the reload shape a tab actually has, one fold after a promotion, is ADR-0088's. (This sentence used to cite the observation note that recorded the failure; the note was retired on 2026-09-26 once the test carried its signal, and only the citation changed.)
>
> Section 1's HEADLINE claim is unchanged and is what survives: the source is the durable REGISTRY and never this process's array order. What changed is only which question is asked of it, and the answer is now the one expression both containers share.
