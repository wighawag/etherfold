...
- **The reader is wrapped in `readOnlyStream`.** A database error must not make a generation permanently unloadable rather than merely un-followed.

  > **AMENDED 2026-09-07 (ADR-0068).** This originally read "wrapped in `degradingStream` as well as `readOnlyStream`", because the load path called `fetchFrom` with no `try`/`catch` above it. `degradingStream` is DELETED: an unreadable substrate now RAISES and the caller decides, and the load path catches (`IndexerGeneration.readStoredStream`). The guarantee is unchanged -- a generation still degrades to a re-index rather than becoming unloadable -- it is reached at the caller that owns the policy instead of at the seam, because a keeper cannot know whether its caller answers absence by re-indexing or by writing.
