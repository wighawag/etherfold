...
## Amendment, 2026-09-26 (ADR-0094, `node-is-a-command-that-receives-uploads`): the mode is `etherfold node`

ADR-0094's first change has landed, and it moves this ADR's subject without changing its behaviour:

- **The node started with nothing configured is `etherfold node`**, a command of its own, and no longer a mode of `run`. It takes the chain, the store and the database like `run`, REFUSES the `-p` and `--deployments` flags by name (pointing at `etherfold upload`), refuses `--override` (its starts replace nothing), and does not read `INDEXING_SOURCE`. Everything "What a node with nothing configured does" says is what a `node` does: the same assembly (`prepareWaiting`, `openWaitingFolding`, `StreamFetchers`), the same `cursor.waiting` on `/status`, the same canonical generation instantiated over the contracts its own bundle carries, the same `--promotion` and `--drop-on-promotion`.
- **`run` requires its processor again**, and `run` with neither processor nor source is refused, naming `node`. The Consequences' "ADR-0048 gains one exception" and "Where it lives"'s `resolveRunProcessor` are superseded: the exception is `node`'s row (ADR-0048's ADR-0094 amendment), and `run`'s processor is resolved by `requireRunProcessor`.
- **`run` serves no upload route**, so "The match rule applies to UPLOADS" has no subject left: a `node` never has a configured source, and the branch that matched an upload against one is deleted (ADR-0085's ADR-0094 amendment). An upload carrying different contracts is always a successor on a new stream.
