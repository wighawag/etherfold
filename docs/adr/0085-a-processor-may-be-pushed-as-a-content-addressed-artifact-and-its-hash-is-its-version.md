...
- **No filesystem.** The artifact arrives in the request, so a deployment with no disk, no deployments folder and no processor path can still be reconfigured. This is the shape a browser tab and a Worker have.
- **No cache breaker.** Distinct bytes are a distinct module by construction, so the `?etherfold-reload=` query, which exists only to defeat Node's ESM registry, becomes unnecessary on this path. A `data:text/javascript;base64,...` URL in Node, or a `blob:` URL from `URL.createObjectURL` in a browser, is instantiated without the loader ever consulting a path.
