...
- **readable store / writable store** (`openForReading` / `openForWriting`) — the two shapes a CONSUMER holds this seam as, so that a reader cannot write because the TYPE says so. `ReadableStateStore` is the four reads, the cursor READ, the capability report, the declarations and `migrate` (schema, idempotent, runs on every open, so it must never claim); `WritableStateStore` is that plus the five mutating verbs and a `token`, which is what makes the type unforgeable — without it any store would structurally satisfy it — and which is deliberately NOT the writer token the backend compares, since a token a caller can read is one a caller can hand back. `openForWriting` migrates and then CLAIMS, so a displaced writer is refused from the moment it returns rather than from whenever the new writer first writes; it does not block and does not wait; and it is IDEMPOTENT per store INSTANCE, because the shipped `createState: () => store` pattern hands one store to every generation and independent claims would have a process refuse itself. The claim is taken by clearing `WRITER_CLAIM_KEY`, a cursor-port key nothing ever writes, which is how the concrete backend classes stay unchanged. The split is at the INTERFACE: a backend implements the whole surface, and `StateStore` is that whole surface — it still carries the mutating half while consumers migrate, and loses it when they have. See ADR-0077.
