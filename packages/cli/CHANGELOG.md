...
- 8a91e65: **`etherfold upload` deploys an already-built processor bundle to a running node, and exits non-zero on anything short of a registration** (ADR-0085).
...
- 0d96749: **`etherfold node` RECEIVES uploads, and `etherfold run` stops receiving them** (ADR-0094). Each command has ONE source of truth: `run` folds what its configuration names, and `node` folds what was uploaded to it.
...
- fb43335: Docs: the CLI README points at `examples/node-dev-loop`, a runnable dev loop (anvil, a contract, `etherfold node`, and `as-soon` rebuilding and uploading the processor on every save).
...
- 55c9680: What an operator reads first, fixed after a hand smoke test of `node` + `upload` against a local chain:
...
- Updated dependencies [0a53b98]
  ...
  - @etherfold/platform-nodejs-fetcher@0.2.0
