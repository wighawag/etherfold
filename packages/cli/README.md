...
ADMIN_TOKEN=dev etherfold upload ./dist/processor.bundle.js --to http://localhost:2000 --indexer default
```
...
| `--override` | let this START replace a DIFFERENT pending successor without asking, exactly as on `run`. A re-run `build -p` against a database whose `successor` slot holds another generation (an upload still catching up, say) would delete it, so without this flag it asks at a terminal and is refused anywhere else |
...
| `--override` | let this START replace a DIFFERENT pending successor without asking, exactly as on `run`: an `index -p` whose processor differs from the canonical one registers it into `successor`, deleting what that slot held, so without this flag it asks at a terminal and is refused anywhere else |
...
| variable | flag | |
| --- | --- | --- |
| `INDEXING_SOURCE` | `-d, --deployments` | what to index, as JSON (`{chainId, contracts}`) where the flag is a deployments folder. Not read by `node`, whose uploads carry their own contracts |
| `ETH_NODE_URI` | `-n, --node-url` | the chain's JSON-RPC endpoint |
| `DB` | `--db` | the libSQL database |
| `PORT` | `--port` | the port an HTTP surface binds |
| `INDEXER_NAME` | `--indexer` | the NAMED INDEXER: what `fetch` pushes into and `index` hosts (required on both), what `upload` addresses (required), and what the stream `run`, `node` or `build` stores is keyed on (optional there, defaulting to `default`) |
...
| `PROMOTION_POLICY` | `--promotion` | WHEN a successor takes over answering reads: `on-catch-up` (the default), `immediate` or `manual`. `run` and `node` only |
