...
- **`stratagems-alpha1.seed.json.gz`** — the published stream SEED emitted from that capture: a `StreamSeed` (`@etherfold/core`), which is the same 31,332 logs stripped to the STORED half, plus the RESOLVED stream config, the stream digest as a label a client verifies, the coverage claimed, the seed's own stored context, the chain head observed at capture and a typed producer declaration. One compact gzipped document, 0.53 MiB against 20.11 MiB of JSON, which is the shape and the size `work/notes/findings/what-a-published-stream-seed-costs-to-install.md` measured. It is a DELIVERABLE and lives in exactly one place: the seed loader and its admission checks install this very file. Re-emit it with `pnpm --filter @etherfold/conformance-workload-stratagems emit:seed` (`../scripts/emit-stream-seed.ts`), which prints the stream digest and the content hash a build would pin; the emit is deterministic, so a re-emit of an unchanged capture produces no diff. `../test/reference-seed.test.ts` is what keeps it honest, including that its content hash reproduces from the committed bytes over the domain ADR-0066 pins (SHA-256 over the DECOMPRESSED octets).
