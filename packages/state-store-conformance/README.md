...
That the capability cases are real is itself a test: `test/the-suite-catches.test.ts` runs the suite against backends with one lie each (claiming a window it does not honour, answering an as-of read from the tip, accepting a revert without undoing it, moving the sync cursor before the block instead of with it, claiming a single writer while letting a second one write, rewinding to make room for a block the tip has passed) and asserts which cases go red. Without that, the capability tests would be decoration.
