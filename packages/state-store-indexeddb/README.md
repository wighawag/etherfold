...
- **A block's height must be ABOVE the recorded tip**, not merely unrecorded, and the tip is read from `blocks` INSIDE that same transaction (one cursor over the primary key, backwards). That is what makes it a compare-and-swap rather than a read-then-write: another tab's revert cannot lower the tip between the read and the write. An offer at or below the tip is a writer working from a position the store has passed, which a backgrounded tab resuming on a stale cursor is the ordinary way to become; an EMPTY store has no tip and admits any height.
